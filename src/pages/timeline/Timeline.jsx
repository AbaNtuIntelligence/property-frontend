...
const RoomDimensions = ({ dimensions, totalArea }) => {
    ...
    const roomConfig = {
        living_room: { icon: '🛋️', label: 'Living Room' },
        master_bedroom: { icon: '👑', label: 'Master Bedroom' },
        bedroom_2: { icon: '🛏️', label: 'Bedroom 2' },
        bedroom_3: { icon: '🛏️', label: 'Bedroom 3' },
        kitchen: { icon: '🍳', label: 'Kitchen' },
        dining_room: { icon: '🍽️', label: 'Dining Room' },
    ...
    return (
        <div className="room-dimensions-wrapper" style={{ margin: '12px 0' }}>
            <button 
            ...
            >
                <span>📐</span>
                <span style={{ flex: 1, textAlign: 'left', fontWeight: 500 }}>Room Dimensions</span>
                {totalArea && <span style={{ fontSize: '0.75rem', color: '#2d6a4f' }}>{totalArea} m² total</span>}
                <span>{expanded ? '▲' : '▼'}</span>
            </button>
                    ...
                    {validRooms.map(([key, dim]) => {
                        ...
                        return (
                            <div key={key} style={{
                                ...
                                <div style={{ flex: 1 }}>
                                    <div style={{ fontWeight: 500, fontSize: '0.85rem' }}>{config.label}</div>
                                    <div style={{ fontSize: '0.7rem', color: '#64748b' }}>{dim.length} × {dim.width} m</div>
                                </div>
                                <div style={{ fontSize: '0.8rem', fontWeight: 600, color: '#2d6a4f' }}>{area} m²</div>
                            </div>
                        );
                    })}
                </div>
